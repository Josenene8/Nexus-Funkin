...
class PlayState extends MusicBeatState
{
	...
	override public function update(elapsed:Float)
	{
		...
		// Credits to namechangedduetononoword, from the fnf mod discord server
		/*if (FlxG.keys.justPressed.SPACE)
			{
				boyfriend.animation.play('singDOWN-dodge', false, false, 24);
				thingy = true;
			...
			if (curSong == 'Insane-Combat' && curStep == 4)
			{
				var lamp:FlxSprite = new FlxSprite(0, 100).loadGraphic(Paths.image('nevada/metalPole'));
				add(lamp);
				FlxTween.tween(lamp, {x: 1080, y: 0}, 1, {ease: FlxEase.linear});
	...
	function goodNoteHit(note:Note):Void
	{
		if (!note.wasGoodHit)
		{
			...
			if(!note.noAnimation) {
				...
				// HARD CODING CAUSE IM STUPIII x2
				// IDK WHAT I DID HERE BUT IT WORK, IDK IF ALT ANIMS WORK AFTER DIS SORRY
				var animToPlay:String = '';
				switch (Std.int(Math.abs(note.noteData)))
				{
					case 0:
						animToPlay = 'singLEFT-dodge';
					case 1:
						animToPlay = 'singDOWN-dodge';
					case 2:
						animToPlay = 'singUP-dodge';
					case 3:
						animToPlay = 'singRIGHT-dodge';
				}
                // copypasted but you already know that HIHIHIHA
